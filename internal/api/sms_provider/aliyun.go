...
func (p *AliyunProvider) SendSms(phone, templateCode, otp string) (string, error) {
	// 构建模板参数，将 OTP 作为模板变量
	// valid for 5min, set OTP_EXP to 5*60=300s
	min := strconv.Itoa(int(p.OptExp / 60))
	templateParam := fmt.Sprintf(`{"code":"%s",min:"%s"}`, otp, min)
...
func (p *AliyunProvider) VerifyOTP(phone string, otp string) error {
	// Skip aliyun SMS verification OTP generation
	// 阿里云短信服务不支持直接的 OTP 验证
	// 这个方法是接口要求的，但实际上不实现
	return fmt.Errorf("OTP verification not supported by Aliyun SMS provider")
}

func NewAliyunProvider(config conf.AliyunProviderConfiguration, optExp uint) (SmsProvider, error) {
